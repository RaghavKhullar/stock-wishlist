...
btn.addEventListener("click",function(){
    ...
    else{stock_data=JSON.parse(check_data);}
...
function t(i,stockname){
    ...
    cell2.innerHTML=`<button id="update" onclick=update_data(${i}) >UPDATE</button>
...
function update_data(i){
    if(click==0){alert ("Click on the UPDATE button of the stock with which you want to update the selected stock");
index=i;
++click;}
else{var check_data=localStorage.getItem('data');
var stock_data=JSON.parse(check_data);
